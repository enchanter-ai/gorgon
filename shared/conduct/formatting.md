...
Audience: Claude. How to shape prompts and responses so the output is parseable on the first try. Root `CLAUDE.md` says *"format follows model"* — this module is the *how*.
...
Do not ship a format to the wrong family without `/translate-prompt`. Root calls this an anti-pattern for a reason.
...
**Compatibility:** prefill is unsupported on some newer models (verify against `shared/models-registry.json`). When unsupported, fall back to structured-outputs API or a system-prompt clause: *"Begin your response with `{`."*
...
- **XML in a GPT prompt without `/translate-prompt`.** Format-model mismatch.
